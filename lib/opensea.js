...
async function fetchHighestOffer(chain, contract, tokenId, collectionSlug) {
  // 1. Best individual offer (bid) for this NFT
  if (collectionSlug) {
    const data = await tryFetch(
      `${OPENSEA_API_BASE}/offers/collection/${collectionSlug}/nfts/${tokenId}/best`
    );
    if (data?.price?.current?.value) return priceFromCurrent(data.price.current);
  }

  // 2. Best listing (ask) for this NFT
  if (collectionSlug) {
    const data = await tryFetch(
      `${OPENSEA_API_BASE}/listings/collection/${collectionSlug}/nfts/${tokenId}/best`
    );
    if (data?.price?.current?.value) return priceFromCurrent(data.price.current);
  }

  // 3. Seaport offers (bids) via contract+tokenId — no slug required
  const offersData = await tryFetch(
  ...
  if (offersData?.orders?.length > 0) {
    const p = priceFromOrder(offersData.orders[0]);
    if (p.offer > 0) return p;
  }

  // 4. Seaport listings (asks) via contract+tokenId — no slug required
  const listingsData = await tryFetch(
    `${OPENSEA_API_BASE}/orders/${chain}/seaport/listings` +
      `?asset_contract_address=${contract}&token_ids=${tokenId}` +
      `&order_by=eth_price&order_direction=desc&limit=1`
  );
  if (listingsData?.orders?.length > 0) {
    const p = priceFromOrder(listingsData.orders[0]);
    if (p.offer > 0) return p;
  }
