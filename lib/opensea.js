...
async function fetchHighestOffer(chain, contract, tokenId, collectionSlug) {
  ...
  if (offerResult?.price?.value) {
    // Prefer the raw Seaport offer amount; OpenSea's price.value can double-count
    // by summing both sides of the order for collection-wide bids.
    const seaportOfferItem = offerResult.protocol_data?.parameters?.offer?.[0];
    if (seaportOfferItem?.startAmount) {
      offer = parseFloat(seaportOfferItem.startAmount) / Math.pow(10, offerResult.price.decimals ?? 18);
      currency = offerResult.price.currency ?? 'WETH';
    } else {
  ...
  const listingPrice = listingData?.price?.value
    ? listingData.price
    : listingData?.price?.current;
  if (listingPrice?.value) {
    ({ offer: listing, currency: listing_currency } = priceFromCurrent(listingPrice));
  }
