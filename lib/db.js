...
function getSql() {
  if (!_sql) {
    ...
    if (!url) throw new Error('No database URL found. Set POSTGRES_URL or DATABASE_URL in environment variables.');
    _sql = neon(url);
  }
  return _sql;
}
...
async function ensureSchema() {
  ...
  `;
  await sql`ALTER TABLE nfts ADD COLUMN IF NOT EXISTS offer_expires_at TIMESTAMPTZ`;
  await sql`ALTER TABLE nfts ADD COLUMN IF NOT EXISTS cheapest_listing NUMERIC DEFAULT 0`;
  await sql`ALTER TABLE nfts ADD COLUMN IF NOT EXISTS listing_currency TEXT DEFAULT 'ETH'`;
}
...
async function updateOffer(id, offer, currency, expires_at, listing, listing_currency) {
  await getSql()`
    UPDATE nfts
    SET current_highest_offer = ${offer},
        offer_currency = ${currency},
        offer_expires_at = ${expires_at ?? null},
        cheapest_listing = ${listing ?? 0},
        listing_currency = ${listing_currency ?? 'ETH'},
